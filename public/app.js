...
const saveSuccessDialog = document.querySelector("#save-success-dialog");
...
let latestRefreshInFlight = null;
...
async function init() {
  ...
  registerResumeRefresh();
}

async function loadLatestParking({ showLoading = true } = {}) {
  ...
  latestRefreshInFlight = (async () => {
    ...
    showView("parking");
      ...
      stopAgeTimer();
      if (showLoading) showView("error");
    } finally {
      lastLatestRefreshAt = Date.now();
...
function registerResumeRefresh() {
  const refreshWhenActive = () => {
    if (document.visibilityState !== "visible") return;
    if (Date.now() - lastLatestRefreshAt < 750) return;
    void loadLatestParking({ showLoading: false });
  };
...
function showSaveSuccessDialog() {
  ...
  saveSuccessDialog.classList.remove("hidden");

  saveSuccessTimer = window.setTimeout(() => {
    saveSuccessDialog.classList.add("hidden");
